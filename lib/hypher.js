...
/**
 * Hyphenates a URL or path like string by placing
 * a zero width space after one or more slashes. This
 * normally would also hyphenate fractions, but short
 * fractions should be filtered out by the minimum word
 * length, and hyphenating long ones is actually
 * desirable. For the rare cases where there is more than
 * one consecutive slash, a zero width space is inserted
 * after each sixth slash.
 *
 * @param {!string} str The path-like string to hyphenate
 * @return {!string} The same string with zero width spaces inserted after each group of six or less slashes.
 */
Hypher.prototype.hyphenateUrl = function (str) {
    return str.replace(/(\/{1,6})/g, function (str, slashes, offset, s) {
        return offset === 0 ||
               offset === s.length - 1 ?
                    slashes :
                    slashes + '\u200B';
